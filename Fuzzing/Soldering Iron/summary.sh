...
qemu_params="./Soldering_Iron.elf -sram_size 256k -flash_size 68k -sram2_base 0 -sram2_size 4k -cpu cortex-m3 -machine embedded_fuzz"
...
do
  ...
  out=$(eval "$EMBER_BASE_DIR/AFLplusplus/afl-qemu-trace -d exec,nochain -fuzz_input summary/crashes/$file -kernel $qemu_params 2>&1 | grep -e 'Trace' -e 'Chain' | tail -1")
...
$(eval "$EMBER_BASE_DIR/AFLplusplus/afl-qemu-trace -fuzz_input summary/queue/$input -kernel $qemu_params -d nochain >/dev/null")
if [ $? -eq 0 ];
then
IFS=$'\n'
for l in $(eval "$EMBER_BASE_DIR/AFLplusplus/afl-qemu-trace -d exec,nochain -fuzz_input summary/queue/$input -kernel $qemu_params 2>&1 >/dev/null | grep '00000000/'")
do
